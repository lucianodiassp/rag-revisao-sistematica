...
def execute_job(job):
    ...
    if job["job_type"] == JOB_VISUAL_INTERPRETATION:
        from backend.app.visual_interpretation import interpret_visual_artifact

        return interpret_visual_artifact(
            project_id,
            parameters.get("artifact_id"),
            progress_callback=callback,
        )
